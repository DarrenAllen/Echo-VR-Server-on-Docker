...
function checkForRunningInstance {
    ...
    pid=$(cat $pidfile)
    echo "checkForRunningInstance pid $pid"
                
    if ps -p $pid > /dev/null; then
        echo "Found process $pid, moving on"
    else
        echo "Process $pid not found, going to start echovr"
...
function checkForError {
    echo "checkForError"
    if ! [ -f /ready-at-dawn-echo-arena/logs/$HOSTNAME/$instance/*.log ]
    then
        return
    fi
    #get the last line of the error file
    lastLine=$(tail -1 /ready-at-dawn-echo-arena/logs/$HOSTNAME/$instance/*.log | cut -c 26- | sed -e s/"[0-9]*\.[0-9]*\.[0-9]*\.[0-9]*:[0-9]*"//g -e s#"ws://.* "##g -e s#" ws://.*api_key=.*"##g -e s/"\?auth=.*"//g)
	...
	then
            ...
            lastLineNew=$(tail -1 /ready-at-dawn-echo-arena/logs/$HOSTNAME/$instance/*.log | cut -c 26- | sed -e s/"[0-9]*\.[0-9]*\.[0-9]*\.[0-9]*:[0-9]*"//g -e s#"ws://.* "##g -e s#" ws://.*api_key=.*"##g -e s/"\?auth=.*"//g)
